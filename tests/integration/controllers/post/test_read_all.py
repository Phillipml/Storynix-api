...
async def populate_posts(db):
    ...
    await service.create(
        PostRequest(title="post 1", content="some content", published=True)
    )
    await service.create(
        PostRequest(title="post 2", content="some content", published=True)
    )
    await service.create(
        PostRequest(title="post 3", content="some content", published=False)
    )


@pytest.mark.parametrize("published,total", [(True, 2), (False, 1)])
async def test_read_posts_by_status_success(
    client: AsyncClient, access_token: str, published: bool, total: int
):
    params = {"published_at": published, "limit": 10}
...
async def test_read_posts_limit_success(client: AsyncClient, access_token: str):
    params = {"published_at": True, "limit": 1}
    headers = {"Authorization": f"Bearer {access_token}"}
